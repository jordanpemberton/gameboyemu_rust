...
impl Mmu {
    ...
    pub(crate) fn read_word(&self, address: u16) -> u16 {
        if BOOT_ROM_BEGIN <= address && address < BOOT_ROM_END {
            ...
            (self.cartridge_rom[(address - CARTRIDGE_ROM_BEGIN) as usize] as u16) << 8
                | self.cartridge_rom[(address - CARTRIDGE_ROM_BEGIN + 1) as usize] as u16
        } else if VRAM_BEGIN <= address && address < VRAM_END {
            (self.vram[(address - VRAM_BEGIN) as usize] as u16) << 8
                | self.vram[(address - VRAM_BEGIN + 1) as usize] as u16
        } else { 0 }
    }
